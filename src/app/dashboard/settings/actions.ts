...
export async function changePasswordAction(_state: unknown, formData: FormData) {
  ...
  const authRows = await db.select().from(clientAuth).where(eq(clientAuth.clientSlug, session.clientSlug)).limit(1)
  ...
  await db.update(clientAuth).set({ passwordHash: hash }).where(eq(clientAuth.clientSlug, session.clientSlug))
